...
const config = {
    A187: 'Rizky Bot', // TOBAT SU ASU
    instagram: 'https://instagram.com/_rizkyfadilah8', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6282387804410', // INFO SU JNGAN DI UBAH
    youtube: 'RIZKY FADILAH', // KINTIL
    whatsapp: 'https://chat.whatsapp.com/LLCSpSL2S8A1NJwTnMIgWY', // BABI
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@_rizkyfadilah8`)
	...
	client.on('message-new', async (mek) => {
		try {
			...
			case 'loli': 
              ...
              costum('Ini', '6282387804410@s.whatsapp.com')
                  break	
				...
				case 'clearall':
					...
					break
				case 'leave': 
					if (!isOwner) return reply('𝗟𝘂 𝘀𝗶𝗮𝗽𝗮?, 𝗻𝘆𝘂𝗿𝘂𝗵 𝗻𝘆𝘂𝗿𝘂𝗵 𝘁𝗼𝗱?') 	
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ 𝙋𝙀𝙎𝘼𝙉 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 👾𝗥𝗜𝗭𝗞𝗬 𝗕𝗢𝗧👾 ❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ 𝙋𝙀𝙎𝘼𝙉 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 👾𝗥𝗜𝗭𝗞𝗬 𝗕𝗢𝗧👾 ❯\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
       ...
       client.sendMessage(from, '𝗧𝘂𝗵 𝗱𝗮𝗵 𝗮𝗸𝘂 𝗸𝗮𝘀𝗶𝗵 𝗻𝗼𝗺𝗲𝗿 𝗖𝗢 👾𝗥𝗜𝗭𝗞𝗬 𝗕𝗢𝗧👾,𝗝𝗔𝗡𝗚𝗔𝗡 𝗟𝗨𝗣𝗔 𝗗𝗜 𝗦𝗔𝗩𝗘 𝗘𝗔 ><',MessageType.text, { quoted: mek} )
           break    
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 𝘁𝗼𝗱🏃 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
